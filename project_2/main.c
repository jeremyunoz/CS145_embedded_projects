...
// Check whether the DST is in effect
int is_dst(DateTime *dt);
...
static volatile int daylight_saving_enable = 0; // check whether the DST is in effect
...
int running_time = 0; // need to keep it or not ?????
...
void advance_dt(DateTime *dt)
{
    ...
    if(isDaylightSavingTime(dt->day, dt->month, dt->year) && !daylight_saving_enable){
        ++dt->hour;
        daylight_saving_enable = 1;
    }


    if(!isDaylightSavingTime(dt->day, dt->month, dt->year) && daylight_saving_enable){
        --dt->hour;
        daylight_saving_enable = 0;
    }
}
...
int is_dst(DateTime *dt) {
    ...
    struct tm march_1st = {0, 0, 0, 1, 2, year - 1900}; // March 1st
    time_t march_1st_time = mktime(&march_1st);
    struct tm *first_sunday_in_march = localtime(&march_1st_time);
    first_sunday_in_march->tm_mday += (7 - first_sunday_in_march->tm_wday) % 7;
    time_t first_sunday_in_march_time = mktime(first_sunday_in_march);
    struct tm second_sunday_in_march = *first_sunday_in_march;
    second_sunday_in_march.tm_mday += 7;
    mktime(&second_sunday_in_march);

    // Find the first Sunday in November
    struct tm november_1st = {0, 0, 0, 1, 10, year - 1900}; // November 1st
    time_t november_1st_time = mktime(&november_1st);
    struct tm *first_sunday_in_november = localtime(&november_1st_time);
    first_sunday_in_november->tm_mday += (7 - first_sunday_in_november->tm_wday) % 7;
    mktime(first_sunday_in_november);

    // Convert DateTime to struct tm for comparison
    struct tm date_tm = {0, dt.second, dt.minute, dt.hour, dt.day, dt.month - 1, year - 1900};

    // DST is in effect if the date is between the second Sunday in March and the first Sunday in November
    time_t date_time = mktime(&date_tm);
    return difftime(date_time, mktime(&second_sunday_in_march)) >= 0 && difftime(mktime(first_sunday_in_november), date_time) > 0;
}
...
void correct_time(DateTime *dt){
    ...
    if(is_dst(dt) && !daylight_saving_enable && dt->hour > 0){
        ++dt->hour;
        daylight_saving_enable = 1;
    }
    if(!is_dst(dt) && daylight_saving_enable && dt->hour > 0){
